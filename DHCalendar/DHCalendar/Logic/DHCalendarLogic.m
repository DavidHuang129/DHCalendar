...
@interface DHCalendarLogic ()
{
    NSDate *today;//今天的日期
    NSDate *after;//之后的日期
    NSDate *appoint; //预约日期;
    DHCalendarDayModel *selectcalendarDay;
    int _index;
}

@property (nonatomic, strong) DHCalendarDayModel *dayModel;
...
@implementation DHCalendarLogic


-(void)setHaveAppointedDays:(NSMutableArray *)haveAppointedDays {
#if __has_feature(objc_arc)
    _haveAppointedDays = haveAppointedDays;
#else
    if(_haveAppointedDays != haveAppointedDays) {
        [_haveAppointedDays release];
        _haveAppointedDays = [haveAppointedDays retain];
    }
#endif
}
- (NSMutableArray *)reloadCalendarView:(NSDate *)date selectDate:(NSDate *)selectDate needDays:(int)days showType:(DHCalendarShowType)type isEnable:(BOOL)isEnable selectDaysArr:(NSMutableArray *)arr optionDays:(int)optionDays {
    ...
    return [self reloadCalendarView:date selectDate:selectDate needDays:days showType:type isEnable:isEnable selectDaysArr:arr];
}
...
- (NSMutableArray *)reloadCalendarView:(NSDate *)date selectDate:(NSDate *)selectDate needDays:(int)days showType:(DHCalendarShowType)type isEnable:(BOOL)isEnable selectDaysArr:(NSMutableArray *)arr {
    ...
    return [self reloadCalendarView:date selectDate:selectDate needDays:days showType:type selectDaysArr:arr];
}
- (NSMutableArray *)reloadCalendarView:(NSDate *)date selectDate:(NSDate *)selectDate needDays:(int)days showType:(DHCalendarShowType)type  selectDaysArr:(NSMutableArray *)arr{
    self.selectDaysArr = arr;
    return [self reloadCalendarView:date selectDate:selectDate needDays:days showType:type];
}
//计算当前日期之前几天或者是之后的几天（负数是之前几天，正数是之后的几天）
- (NSMutableArray *)reloadCalendarView:(NSDate *)date selectDate:(NSDate *)selectDate needDays:(int)days showType:(DHCalendarShowType)type {
    //如果为空就从当天的日期开始
    if(date == nil){
        date = [NSDate date];
    }
    _index = 0;
    if(self.haveAppointedDays == nil){
        self.haveAppointedDays = [NSMutableArray array];
    }
    today = date;//起始日期
    
    after = [date dayInTheFollowingDay:days];//计算它days天以后的时间
    
    appoint = [date dayInTheFollowingDay:self.optionDays]; //计算预约之后的时间
    ...
    for (int i = 0; i <= months; i++) {
        ...
        if(type == DHCalendarShowTypeMultiple){
...
- (NSMutableArray *)calculateDaysInPreviousMonthWithDate:(NSDate *)date andArray:(NSMutableArray *)array
{
    ...
    for (int i = daysCount - partialDaysCount + 1; i < daysCount + 1; ++i) {
        ...
        DHCalendarDayModel *calendarDay = [DHCalendarDayModel calendarWithYear:components.year month:components.month day:i];
...
- (void)changStyle:(DHCalendarDayModel *)calendarDay
{
    
    NSDateComponents *calendarToDay  = [today YMDComponents];//今天
    NSDateComponents *calendarAfterDay = [after YMDComponents];//最后一天
    NSDateComponents *calendarAppointDay = [appoint YMDComponents]; //截止日期
    
    NSDateComponents *todayHourMinSec = [today HMSComponents];
    NSDate *tomorrow = [today dateByAddingTimeInterval:24.0f*60.0f*60.0f]; //明天
    NSCalendar *gregorian = [[NSCalendar alloc]
                             initWithCalendarIdentifier:NSCalendarIdentifierGregorian];
    NSDateComponents *calendarCompare = [[NSDateComponents alloc]init];
    calendarCompare.year = calendarDay.year;
    calendarCompare.month = calendarDay.month;
    calendarCompare.day = calendarDay.day;
    calendarCompare.hour = todayHourMinSec.hour;
    calendarCompare.minute = todayHourMinSec.minute;
    calendarCompare.second = todayHourMinSec.second;
    calendarCompare.week = calendarDay.week;
    NSDate *compareDate = [gregorian dateFromComponents:calendarCompare];
    
    //今天乃至过去都属于 CellDayTypePast
   if (today.timeIntervalSince1970 >= compareDate.timeIntervalSince1970) {
        calendarDay.style = CellDayTypePast;
   }else if (today.timeIntervalSince1970 < compareDate.timeIntervalSince1970 && compareDate.timeIntervalSince1970 < appoint.timeIntervalSince1970){
       calendarDay.selectType = CollectionViewSelectTypeOption;
//       _index == self.optionDays - 1 ? 0:_index++;
//       
//       NSLog(@"****%d",_index);
       calendarDay.style = CellDayTypeOption;
       [self.selectDaysArr addObject:calendarDay];
   }else if(compareDate.timeIntervalSince1970 > appoint.timeIntervalSince1970){
       calendarDay.style = CellDayTypeFutur;
   }

    NSDate *Option30Days = [today dateByAddingTimeInterval:24*60*60*30];
    if (today.timeIntervalSince1970 < compareDate.timeIntervalSince1970 && compareDate.timeIntervalSince1970 < Option30Days.timeIntervalSince1970) {
        calendarDay.isEdit = YES;
        calendarDay.selectType = CollectionViewSelectTypeEmpty;
    }
...
- (NSDateComponents *)dateComponentsFromString:(NSString *)dateString {
    ...
    date1 = [formatter2 dateFromString:dateString];
    NSDateComponents *calendarToDay  = [date1 YMDComponents];
    
    return calendarToDay;
}
#pragma mark - 农历转换函数

-(void)LunarForSolarYear:(DHCalendarDayModel *)calendarDay{
